...
function SalesPersonList() {
  ...
  const [reqSalesPerson, resSalesPerson] = useSalesPersonListMutation();
  const [reqDelete, resDelete] = useDeleteSalesPersonMutation();
  ...
  useEffect(() => {
    if (resSalesPerson?.isSuccess) {
      const filterRes =
        resSalesPerson?.data?.data?.docs &&
        Array.isArray(resSalesPerson?.data?.data?.docs) &&
        resSalesPerson?.data?.data?.docs?.length > 0
          ? resSalesPerson?.data?.data?.docs?.map((el) => ({
              ...el,
      ...
      dispatch(getSalesPerson(filterRes));
    }
  }, [resSalesPerson]);

  const onEditAction = (e, st) => {
    e.preventDefault();
    navigate("/sales-person-form", {
      state: {
        salesPersonID: st?.row?.original?._id,
        isEdit: true,
      },
    });
  };
  ...
  useEffect(() => {
    if (resManageSession?.isSuccess) {
      toast.success(
        `Successfully ${
          selectedSalesPerson?.isDel ? "Deactivate" : "Activate"
        } Sales Person`,
        {
          position: "top-center",
        }
  ...
  return (
    ...
    <>
      ...
      <SalesPersonView modal={modal} setModal={setModal} viewData={viewData} />
      <VerifyDeleteModal
        showModal={showModal}
